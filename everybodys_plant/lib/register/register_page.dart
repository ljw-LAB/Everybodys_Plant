...
class _RegisterPageState extends State<RegisterPage> {
  ...
  TextEditingController _LastWaterDayController =
      TextEditingController(text: '눌러서 날짜를 선택해주세요');

  DateTime? tempPickedDate;
  DateTime _selectedDate = DateTime.now();

  //세진이 수정한 부분(분갈이)
  TextEditingController _LastSoilDayController =
      TextEditingController(text: '눌러서 날짜를 선택해주세요');

  DateTime? tempPickedDatesoil;
  DateTime _selectsoilDate = DateTime.now();
                        ...
                        child: Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton(
                              ...
                              style: const TextStyle(color: Colors.black),
                              items:
                                  spaceitems.map((String spaceitems_map_val) {
                                return DropdownMenuItem(
                        ...
                        child: Container(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton(
                              ...
                              style: const TextStyle(color: Colors.black),
                              items:
                                  perioditems.map((String perioditems_map_val) {
                                return DropdownMenuItem(
                              ...
                              child: TextButton(
                                onPressed: () {
                                  ...
                                  if (nickname.isEmpty) {
                                    setState(() {
                                      error =
                                          "애칭을 입력해주세요."; // 애칭 입력 안 되었을 때 에러 문구
                                    });
                                  } else {
                                    setState(() {
...
///////////////////////////////////////////////물준날 달력 위젯
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            controller: _LastWaterDayController,
  ...
  _selectDate() async {
    ...
    if (pickedDate != null && pickedDate != _selectedDate) {
      setState(() {
        ...
        _LastWaterDayController.text = pickedDate.toString();
        convertDateTimeDisplay(_LastWaterDayController.text);
      });
    }
  }
  ...
  String convertDateTimeDisplay(String date) {
    ...
    return _LastWaterDayController.text = serverFormater.format(displayDate);
  }

///////////////////////////////////////////////분갈이 달력 위젯
  Widget LastSoildayText() {
    return GestureDetector(
      onTap: () {
        HapticFeedback.mediumImpact();
        _selectDatesoil();
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('', style: TextStyle(fontSize: 15, color: Colors.blueGrey)),
          TextFormField(
            enabled: false,
            decoration: InputDecoration(
              isDense: true,
            ),
            controller: _LastSoilDayController,
            style: TextStyle(fontSize: 15),
          ),
        ],
      ),
  ...
  _selectDatesoil() async {
    ...
    if (pickedDate != null && pickedDate != _selectedDate) {
      setState(() {
        _selectedDate = pickedDate;
        _LastSoilDayController.text = pickedDate.toString();
        convertDateTimeDisplay2(_LastSoilDayController.text);
      });
    }
  }
  ...
  String convertDateTimeDisplay2(String date) {
    ...
    final DateFormat serverFormater = DateFormat('yyyy-MM-dd');
    final DateTime displayDate = displayFormater.parse(date);
    return _LastSoilDayController.text = serverFormater.format(displayDate);
  }
}
